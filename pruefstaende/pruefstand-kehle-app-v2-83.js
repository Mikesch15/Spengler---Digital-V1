...
(async()=>{
 ...
 const zu=await page.evaluate(()=>{
  ...
  return {art:plan.art,breiten:plan.streifenbreiten,kenn,
   bestes:rp.bestes,netto:Number(rp.netto.toFixed(4)),tafel:rp.tafelLaenge,
 ...
 p(zu.tafel===2070,"die Tafellaenge ist das laengste Stueck (2070 mm)",zu);
 p(Math.abs(zu.netto-2.7965)<1e-4,"Blech netto = 5593 mm x 500 mm = 2.80 m2",zu.netto);
 p(zu.bestes&&zu.bestes.breite===1000&&zu.bestes.jeTafel===2&&zu.bestes.tafeln===2,
   "beste Rolle 1000 mm: 2 Streifen je Tafel, 2 Tafeln",zu.bestes);
 p(zu.bestes&&Math.abs(zu.bestes.flaeche-4.14)<1e-6&&Math.abs(zu.bestes.verschnitt-1.3435)<1e-4,
   "4.14 m2 Tafelflaeche, 1.34 m2 Verschnitt",zu.bestes);
 ...
 const geteilt=await page.evaluate(()=>{
  ...
  const st=(rp.verteilung&&rp.verteilung.streifen)||[];
 ...
 p(d.rollen&&d.rollen.bestes&&d.rollen.bestes.breite===1000&&d.rollen.tafelLaenge===2070,
   "neu: der Rollenblech-Plan",d.rollen&&d.rollen.bestes);
 p(d.rollen&&d.rollen.verteilung&&Array.isArray(d.rollen.verteilung.streifen)
   &&d.rollen.verteilung.streifen.length===3,"mit der Belegung der Streifen",
   d.rollen&&d.rollen.verteilung&&d.rollen.verteilung.streifen&&d.rollen.verteilung.streifen.length);
