"use strict";
// ---- Pruefstand: thinkingBudget:0 + maxOutputTokens:65536 (v3.41) ---------
//
// Auftrag: nachdem v3.40 den 502-Fehler bei einer sehr dichten, echten
// 90-Positionen-Offerte durch eine EHRLICHE, konkrete Fehlermeldung ersetzt
// hatte (statt eines nichtssagenden Fehlers), hat der Betrieb ein
// Bildschirmfoto geschickt, das genau diese neue Meldung im Betrieb zeigt,
// und gefragt: "koennen wir das verbessern?" - also nicht nur sauber
// scheitern, sondern GROSSE Dokumente tatsaechlich erfolgreich verarbeiten.
//
// Recherche (Websuche gegen Googles eigene Gemini-API-Dokumentation/-Forum,
// da diese Sandbox keinen echten Live-Aufruf machen kann, um es empirisch zu
// bestaetigen - "gemessen, nicht vermutet" gilt hier als "recherchiert,
// nicht geraten"):
//   1. MODEL ist ein Gemini-3.x-Flash-Modell. Google dokumentiert selbst:
//      "Gemini 3 Flash and Flash-Lite also do not support full
//      thinking-off" - auf dieser Modellgeneration kann unsichtbarer
//      "Denk"-Token-Verbrauch in denselben maxOutputTokens-Topf greifen,
//      den JSON.parse() fuer die eigentliche Positionsliste braucht, obwohl
//      diese Aufgabe (eine Tabelle mechanisch ablesen) keinerlei erweitertes
//      Schlussfolgern braucht. Das war ein in v3.40 nicht beruecksichtigter
//      Token-Verbraucher: ein Teil der "8192 Token Reserve" stand der
//      eigentlichen JSON-Ausgabe womoeglich nie zur Verfuegung.
//      Fix: thinkingConfig:{thinkingBudget:0} wird jetzt ausdruecklich
//      gesetzt, um das so weit zu minimieren, wie das Modell es zulaesst.
//   2. maxOutputTokens erneut angehoben, von 8192 auf 65536 - keine erneute
//      Ratezahl, sondern die von Google dokumentierte Obergrenze fuer diese
//      Gemini-Generation (Gemini 2.5 Pro: bis zu 65535/65536 Ausgabe-Token;
//      Flash-Modelle derselben Generation teilen diese Obergrenze). Bei
//      rund 33 Token je Position ergibt das rund 1985 Positionen Reserve -
//      noch bevor thinkingBudget:0 zusaetzlichen Platz freimacht.
// Der ehrliche MAX_TOKENS-Rueckfall aus v3.40 bleibt UNVERAENDERT als
// Sicherheitsnetz bestehen (CLAUDE.md §78.5: niemals Positionen erfinden
// oder ein abgeschnittenes Array teilweise uebernehmen) - er sollte jetzt
// nur praktisch nie mehr ausgeloest werden.
//
// Bewusst NICHT gebaut: PDF-Seitenaufteilung/mehrere Gemini-Aufrufe
// nacheinander. Das waere eine deutlich groessere, riskantere
// Infrastruktur (eine PDF-Teilungsbibliothek in der Deno-Laufzeit, mehrere
// sequentielle Aufrufe, Zusammenfuehrungs-/Duplikat-Logik, Teilfehler-
// Behandlung) fuer ein Problem, das die einzelne, jetzt weit hoehere Grenze
// bereits mit sehr grosser Reserve abdeckt (Schweizer NPK-Offerten haben
// selten mehr als ein paar hundert Positionen) - siehe CLAUDE.md's eigene
// Hausregel gegen das Bauen von Infrastruktur fuer einen nicht konkret
// nachgewiesenen Bedarf.
//
// Die Edge Function wurde bereits ueber mcp__Supabase__deploy_edge_function
// live deployt (Version 13, status:ACTIVE, bestaetigt ueber
...
//       recognizePhoto() (js/17-ausmass.js) reicht jeden vom Server
//       gelieferten error-String unveraendert weiter; ein erfolgreicher,
//       sehr grosser Treffer (weit ueber die alte 8192er-Grenze hinaus)
//       muss VOLLSTAENDIG ankommen, ohne dass diese Sandbox dafuer
//       irgendeine Client-Datei aendern musste.
//
// Geprueft:
//  1  Struktur: maxOutputTokens ist 65536, nicht mehr 8192
//  2  Struktur: thinkingConfig:{thinkingBudget:0} ist gesetzt
//  3  Struktur: 8192 kommt im tatsaechlichen Code (Kommentare entfernt)
//     nirgends mehr als maxOutputTokens-Wert vor
//  4  Struktur: das alte, generische maxOutputTokens:8192-Literal ist NICHT
//     mehr Teil der generationConfig (praezise Pruefung des Objekts)
...
//  8  Struktur: resolveImage()/bytesToBase64() sind unveraendert vorhanden -
//     kein zweiter Erkennungsweg
//  9  Verhalten: ein sehr grosser, erfolgreicher Treffer (400 Positionen -
//     bei der alten 8192er-Grenze rechnerisch unmoeglich, bei 65536 bequem
//     im Rahmen) wird VOLLSTAENDIG uebernommen, ohne jede clientseitige
//     Kappung
// 10  Verhalten: der MAX_TOKENS-Fall (Sicherheitsnetz) erreicht die Person
//     weiterhin unveraendert ueber denselben Mechanismus wie in v3.40 -
//     OHNE dass irgendeine Position erfunden wird
// 11  Verhalten: der alte, generische Fehlerfall bleibt unveraendert (Regres-
//     sionsschutz)
// 12  Struktur: recognizePhoto() bleibt Unikat in js/17-ausmass.js
// 13  Struktur: keine geschuetzte Fachdatei wurde fuer diesen rein server-
//     seitigen Fix angefasst (git diff --name-only HEAD)
//
// Gegenprobe (siehe CLAUDE.md Abschnitt 88.8): separat als eigener Vorgang
// durchgefuehrt (Baum sichern, echten Fehler einbauen - thinkingConfig
// entfernen UND maxOutputTokens zurueck auf 8192 setzen -, diesen
// Pruefstand erneut laufen lassen, genau den erwarteten Fehlschlag
// bestaetigen, wiederherstellen). Kein Umschalter dafuer in dieser Datei.
//
// Aufruf:  SP=<Ordner mit node_modules> node pruefstaende/pruefstand-thinking-budget-v3-41.js
...
p(/maxOutputTokens:\s*65536/.test(quelltextEdge),"maxOutputTokens ist 65536 (nicht mehr 8192)");
...
(async()=>{
 ...
 let antwortModus="grosser_erfolg";
 await page.route("**/functions/v1/extract-offer-positions",async route=>{
  ...
  if(antwortModus==="max_tokens"){
   ...
   // 400 Positionen: bei der ALTEN 8192er-Grenze rechnerisch unmoeglich
   // (400*33 ≈ 13200 Token, weit ueber 8192), bei der neuen 65536er-Grenze
   // bequem im Rahmen (400*33 ≈ 13200, weit unter 65536).
   const positions=Array.from({length:400},(_,i)=>({pos:String(i+1),description:"Position "+(i+1),quantity:i+1,unit:"m2"}));
 ...
 console.log("\nB · Verhalten - sehr grosser Treffer (400 Positionen, bei der alten Grenze unmoeglich)");
 ...
 await page.locator("#angPdfInput").setInputFiles({
  name:"sehr-grosse-offerte-400-positionen.pdf",mimeType:"application/pdf",
 ...
 p(grossErgebnis.anzahlPositionen===400,
  "alle 400 vom (jetzt weit hoeheren Token-Limit ermoeglichten) erfolgreichen Treffer gelieferten Positionen werden VOLLSTAENDIG uebernommen - keine clientseitige Kappung",grossErgebnis);
 p(/400 Position\(en\) aus dem PDF erkannt/.test(grossErgebnis.status),"und die Statuszeile nennt die volle Anzahl",grossErgebnis);
 ...
 p(maxTokensErgebnis.anzahlPositionen===0,
  "bei diesem (jetzt sehr unwahrscheinlichen) Fall wird weiterhin KEINE Position erfunden",maxTokensErgebnis);
