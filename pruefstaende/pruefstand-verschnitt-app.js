// Prueft raNormPlan() gegen eine unabhaengige, vollstaendige Suche.
// Aufruf:  node pruefstaende/pruefstand-verschnitt-app.js  (aus dem Repo-Wurzelverzeichnis)
// Geprueft wird die Fassung, die in der App laeuft (js/28-rinne-aufnahme.js).
...
const src=fs.readFileSync("js/28-rinne-aufnahme.js","utf8");
const von=src.indexOf("const RA_NORM_VORGABE");
const bis=src.indexOf("// ---- Zustand");
const ctx={console,localStorage:{getItem:()=>null,setItem:()=>{},removeItem:()=>{}},
 raZahl:v=>{const n=Number(v);return Number.isFinite(n)?n:0},Math,JSON,Array,Number,String,Object};
...
function pruefe(stuecke,normen,name){
 const r=ctx.raNormPlan(stuecke,normen);
...
r=ctx.raNormPlan([7000,3000],[6000]);
...
r=ctx.raNormPlan(gross,[4000,5000,6000]);
...
console.log(`\npruefstand-verschnitt-app: ${ok}/${ok+fail}`+(fail?"  FEHLGESCHLAGEN":"  - alle bestanden"));
