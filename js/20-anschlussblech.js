...
const ANB_ARTEN = Object.freeze({
  ...
  pv_seite: {
    // Kein Wandanschluss: senkrechter Schenkel an der PV-Schiene, oben
    // zwei Umschläge nacheinander (erst d, dann e), dann waagerecht frei
    // unter das Deckmaterial durch.
    name: "PV-Seitenanschluss",
    masse: {
      a: { std: 50, min: 50, text: "Blech bis Deckmaterial" },
      b: { std: 50, min: 50, text: "Überdeckung des Bleilappens" },
      c: { std: 80, min: null, text: "Höhe senkrechter Schenkel bis PV-Schiene" },
      d: { std: 20, min: null, text: "Erster Umschlag oben am Schenkel" },
      e: { std: 20, min: null, text: "Zweiter Umschlag oben am Schenkel" }
    }
  }
});
...
function anbProfil(e) {
  ...
  const a = z(e.a), b = z(e.b), c = z(e.c), d = z(e.d), eMass = z(e.e);
  ...
  if (art === "bleilappen") {
    ...
    // Senkrechter Schenkel bei x=0 (PV-Schienen-Seite), oben zwei echte
    // Umschläge nacheinander (erst d, dann e) – als eigene Geometrie
    // gezeichnet, gleich wie der doppelt gekantete Steg. Der kleine
    // seitliche Versatz (steg) ist nur damit sich die beiden Falze in
    // der Zeichnung nicht überdecken. Danach waagerecht unter das
    // Deckmaterial durch, dort ohne Umschlag.
    pts = [
      [-2 * steg, c - d + eMass],
      [-2 * steg, c - d],
      [-steg, c - d],
    ...
    teile.push({ name: "Seitenblech", pts: pts });
...
function anbZeichnung(e) {
  ...
  const a = Number(e.a) || 0, b = Number(e.b) || 0, c = Number(e.c) || 0, d = Number(e.d) || 0, eMass = Number(e.e) || 0;
  ...
  if (e.art === "bleilappen") {
    ...
    g += waag(0, a, "a = " + zahl(a)) + waag(a, a + b, "b = " + zahl(b));
    g += anbFahne(0, c, 26, -8, "d = " + zahl(d), X, Y);
    g += anbFahne(0, c - d, 40, 16, "e = " + zahl(eMass), X, Y);
  }
...
function berechneAnschlussblech(e) {
  ...
  // Stückliste – jedes Segment wird für sich aufgeteilt, gleiche Regel
  // wie beim Einlaufblech: volle Stücke zur eingestellten Stücklänge,
  // am Schluss des Segments das Reststück. Die Überlappung kommt beim
  // vollen Stück auf den Zuschnitt; das Reststück braucht keine, weil
  // das Stück davor darüber greift. Ein Reststück unter der Schwelle
  // wird dem vorherigen Stück im selben Segment zugeschlagen. Segmente
  // greifen nicht ineinander über – kein Überlappungsmass zwischen zwei
  ...
  const segmenteGueltig = (Array.isArray(e.segmente) ? e.segmente : [])
    .filter(seg => (Number(seg.laenge) || 0) > 0);
  const stuecke = [];
  segmenteGueltig.forEach((seg, segIdx) => {
    const segLetztes = segIdx === segmenteGueltig.length - 1;
    const segLaenge = Number(seg.laenge) || 0;
    let anzahl = Math.max(1, Math.ceil(segLaenge / stoss));
    let rest = segLaenge - (anzahl - 1) * stoss;
    if (anzahl > 1 && rest > 0 && rest < schwelle) { anzahl -= 1; rest = stoss + rest; }
    for (let i = 0; i < anzahl; i++) {
      const stueckLetztes = i === anzahl - 1;
      const grund = Math.round(stueckLetztes ? rest : stoss + lap);
      const st = { nr: stuecke.length + 1, laenge: grund, gehrung: false };
      if (segLetztes && stueckLetztes && mitGehrung && gehrungZugabe > 0) {
        st.gehrung = true;
...
const ANSCHLUSSBLECH_STANDARD = Object.freeze({
  ...
  rest_schwelle: 300,         // kürzeres Reststück wird dem vorherigen Stück angehängt
